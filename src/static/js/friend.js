const arrow = document.querySelector(".friend_arrow"),
  add_friend = document.querySelector(".js_friend"),
  f_div = document.querySelector(".f_div"),
  f_input = document.querySelector(".friend_id"),
  clear_btn = document.querySelector(".friend_id+span"),
  friend_info = document.querySelector(".friend_info");
(p_s = document.querySelectorAll(".info_form p")),
  (add_btn = document.querySelector(".info_form button")),
  (f_ul = document.querySelector(".friend_ul")),
  (friend_div = document.querySelector(".friend_div"));
function checkArrow(e) {
  const { target } = e;
  if (target.classList.contains("up")) {
    target.classList.remove("up");
    target.classList.add("down");
    target.src = "/static/img/down_arrow.png";
    ...
    target.src = "/static/img/up_arrow.png";
    ...
    fetch(`friend/${target.value}`)
      ...
      .then(data => {
        ...
        if (result["error"] === 0 && result["success"] === 1) {
          ...
          if (
            checkSame(
              document.querySelector(".profile_main .username").textContent,
              p_s[1].textContent
            )
          ) {
...
function handleAdd(e) {
  const { target } = e,
    f_list = document.querySelector(".friend_ul");
  ...
  if (!checkSame(my, p_s[1].textContent)) {
    ...
    fetch(`/add_friend`, options).then(resolve => {
      //console.log(resolve);
    ...
    fetch(`/friend_list/${myid}`)
      ...
      .then(data => {
        ...
        friends.forEach(item => {
          f_ul.appendChild(create_friend_tag(item.email));
        });
      });
  }
}
